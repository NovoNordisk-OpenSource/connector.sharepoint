local_create_sharepoint_directory <- function(site_url = NULL, ..., env = parent.frame()) {
  ...
  con <- connector_sharepoint(site_url = test_sharepoint$site_url, path_of_folder =  test_dir, ...)
...
local_download_sharepoint_file <- function(con = NULL,
                                           file_name = NULL,
                                           env = parent.frame()) {
  con |>
    download_content_cnt(file_name, file_name, overwrite = TRUE)
